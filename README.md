...
- **GitHub Releases**: リポジトリの `Releases` タブから公開版を取得
- **GitHub Actions Artifacts**: `Dev Build` / `Release Build` 実行結果の `dev-package-*` / `release-package-*` から取得
- **開発版（Dev Build）**: `Releases` の `dev-latest` prerelease から取得
