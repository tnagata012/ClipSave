...
| SPEC-000-006 | 同一ユーザー/同一セッションで多重起動試行時、新規インスタンスは終了する |
| SPEC-000-007 | 同一ユーザー/同一セッションで多重起動試行時、既存インスタンスの設定画面を表示する |
...
| ID | 仕様 |
|----|------|
| SPEC-060-001 | About 画面にアプリバージョン（`ReleaseProcess.md` の SSOT `Version` に対応する `X.Y.Z`）、.NET バージョン、OS バージョン、ビルド日時、著作権情報を表示する |
